...
sol :: [String] -> Int
sol s = countScore 0 gameScores
    where
        countScore :: Int -> [(Int, Int)] -> Int
        countScore acc [] = acc
        countScore acc ((count, numMatches):cs) =
            countScore (acc + count) (dupCards numMatches count cs)
        dupCards :: Int -> Int -> [(Int, Int)] -> [(Int, Int)]
        dupCards _ _ [] = []
        dupCards 0 numDupes cards = cards
        dupCards numCardsToDupe numDupes ((count, numMatches):cs) =
            (count + numDupes, numMatches):dupCards (numCardsToDupe-1) numDupes cs
        gameScores = map ((1,) . resolveLine) s
